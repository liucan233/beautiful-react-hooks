...
const useEvent = <TEvent extends Event, TElement extends HTMLElement = HTMLElement>
  ...
  useEffect(() => {
    ...
    const canAddAndRemoveListener=(element instanceof HTMLElement || element===window);

    if (canAddAndRemoveListener && handler.current) {
      element.addEventListener(eventName, cb, options)
    }

    return () => {
      canAddAndRemoveListener && element.removeEventListener(eventName,cb)
    }
